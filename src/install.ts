...
目标：**agent 是无状态的，项目上下文是有状态的。**

### 开场：接上次的活

新会话开场、用户还没给出明确任务，或用户提到「继续 / 接着上次 / 同步历史 / 换个模型 /
之前那个会话 / 上次说的 / handoff」，又或者任务明显在延续某个你手上没有的上下文时：

1. 先看本仓库有没有上下文（MCP: \`session_status\`；命令行: \`ass\`）——
   它会同时给出「上次收尾的摘要」和「之前记录过的决策/踩坑/待办」；
2. 需要更多历史就列出该仓库跨 agent 的最近会话（\`session_list\` / \`ass list\`），
...
这是最容易被跳过、但价值最高的一步。**一旦**发生下面这些事，立刻调用 \`session_remember\`
（命令行：\`ass context --decision|--dead-end|--constraint|--todo "…"\`），不要等收尾：

- 做了架构或方案上的**决策** → 记下「决定了什么 + 为什么」（\`decision\`）
- 试了**走不通**的方案、回滚了某次改动 → 记下来，**别让下一个 agent 再踩一遍**（\`dead_end\`）
- 发现**不能碰**的东西（旧接口、兼容逻辑、私有约定） → 记下来（\`constraint\`）
- 还剩什么没做 → 记成 \`todo\`，做完用 \`session_remember(done: "c4")\` 勾掉

> 为什么强调这条：从原始对话里"猜"当时为什么这么决定很不靠谱，
> 而你亲手记的这一句，会**置顶**出现在之后每一个 agent 的交接摘要里。

### 收尾：把状态落下来

用户说「记一下 / 收尾 / 保存进度 / 下次继续 / wrap up」时：

1. 先把过程中没记的决策/坑补上（\`session_remember\`）；
2. 再保存一份交接摘要（\`session_note\` / \`ass note\`），下次任何 agent 一进这个仓库就能看到。

### 约束

- 默认只给摘要；用户明确要「完整历史 / 全文」时才读全文（\`session_read mode=full\`），
  且不要把整段原文贴进回复 —— 150 轮对话里大部分是来回试错，真正有价值的是最终状态。
- 引用交接摘要里的结论时，要说明它来自「之前某个 agent 的会话」，不要当成你自己验证过的事实；
  涉及代码现状的结论，仍然要用当前代码去核实。
- 不要为了同步去修改其它 agent 的会话文件；本工具全程只读。
- 没有 MCP 工具时，退回命令行：\`ass\` / \`ass list\` / \`ass show "#3"\` / \`ass last "#3"\` / \`ass context\`。
${MARK_END}`
}
